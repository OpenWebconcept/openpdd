...
class GW_Zip_Files
{
    public function __construct($args = [])
    {
        ...
        add_filter('gform_entries_field_value', [ $this, 'modify_zip_display_value' ], 10, 3);

        add_filter('gform_entry_post_save', [ $this, 'archive_files' ], 10, 2);
        add_filter('gform_notification', [ $this, 'add_zip_as_attachment' ], 10, 3);
        add_filter('gform_replace_merge_tags', [ $this, 'all_files_merge_tag' ], 10, 7);
        add_filter('gform_replace_merge_tags', [ $this, 'zip_url_merge_tag' ], 10, 3);
    ...
    public function is_applicable_field($field)
    {
        ...
        $is_applicable_input_type = in_array($input_type, [ 'fileupload', 'post_image' ]);
    ...
    public function get_slug($name, $entry_id = false, $field_ids = [])
    {
    ...
    public function zip_url_merge_tag($text, $form, $entry)
    {
        ...
        if ($zip_file) {
            $zip = new ZipArchive;
            if ($zip->open($zip_file) && 0 < $zip->numFiles) {
    ...
    public function get_all_files_output($entry)
    {
        ...
        $replace = array_merge(
            [ __('Uploaded Files:') ],
            $file_links,
            [ sprintf('<a href="%s">%s</a>', $this->get_zip_paths($entry, 'url'), __('All Files')) ]
        );
                    ...
                    foreach (explode(',', $value) as $nested_entry_id) {
                        ...
                        if ($zip_path) {
                            if (isset($notification['attachments'])) {
                                $notification['attachments'] = is_array($notification['attachments']) ? $notification['attachments'] : [ $notification['attachments'] ];
                            } else {
